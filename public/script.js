// public/script.js - Includes Presets (Renamed) and Landing Page Examples Logic
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const criticalElementRefs = { landingView, appView, imageInput, optionsForm, convertBtn, statusArea, svgOutputDiv, downloadBtn, uploadNewBtn, dropZone, previewAreaWrapper, previewOriginalImage, originalImageWrapper, vectorImageWrapper, modeSelect, colormodeSelect, resetOptionsBtn, startConversionBtn, fileInfoArea, fileNameDisplay, cancelUploadBtn, uploadProgress, progressBar, landingStatusArea, zoomInBtn, zoomOutBtn, zoomResetBtn, paletteSelect, colorPrecisionInput, presetSelect, saveOptionsBtn, loadOptionsBtn, optionsFileInput };
    ...
    const presets = [
        ...
        },
        {
            ...
            options: {
                color_mode: "bw", filter_speckle: "2", // Lowered speckle slightly
        ...
        },
        {
            ...
            options: {
                ...
                palette_selector: "8", color_precision: "8", gradient_step: "5", // Adjusted gradient step
                mode: "spline", corner_threshold: "60", path_precision: "3",
                spline_threshold: "0.75", splice_threshold: "45", segment_length: "4"
            }
        },
        {
            name: "Clipart / Logo", // Was Poster, adjusted
            options: {
                color_mode: "color", hierarchical: "stacked", filter_speckle: "2", // Lower speckle
                palette_selector: "8", color_precision: "8", gradient_step: "1", // Lower gradient
                mode: "spline", corner_threshold: "40", // Slightly lower corner threshold
                path_precision: "3", spline_threshold: "0.75", splice_threshold: "45", segment_length: "2" // Shorter segments
            }
        },
        {
            name: "Cartoon / Flat Style", // Was Landscape, adjusted
            options: {
                color_mode: "color", hierarchical: "stacked", filter_speckle: "2", // Low speckle
                palette_selector: "5", color_precision: "5", // Reduced colors
                gradient_step: "1", // Low gradient
                mode: "spline", corner_threshold: "60", path_precision: "3",
                spline_threshold: "0.75", splice_threshold: "45", segment_length: "4"
            }
        },
        {
            ...
            options: {
                color_mode: "color", hierarchical: "stacked", filter_speckle: "6", // Moderate speckle
                palette_selector: "8", color_precision: "8", gradient_step: "8", // Moderate gradient
                mode: "spline", corner_threshold: "100", // Higher corner threshold
    ...
    // --- Initial UI Setup ---
    showLandingView();
    updateOptionsAvailability(); // Call before storing defaults
    storeDefaultOptions();     // Store defaults AND populate the "General / Balanced" preset
    populatePresetSelect();    // Populate the preset dropdown
    setupZoomPan();
    updateZoomButtons();
    ...
    // Removed individual option listeners, handled by form 'change'
    ...
    // Option form change listener (centralized update logic)
    if (optionsForm) { optionsForm.addEventListener('change', () => { // When any form element changes...
        ...
        // Deselect preset dropdown if user manually changes settings
    ...
    // Preset and Load/Save Listeners
    ...
    async function handleConvert(isInitial = false) { if (!currentFile || !convertBtn || !downloadBtn) return; updateStatus(isInitial ? 'Vectorizing...' : 'Updating...', 'loading'); convertBtn.disabled = true; convertBtn.textContent = 'Working...'; downloadBtn.disabled = true; if (svgOutputDiv) svgOutputDiv.innerHTML = `<p class="placeholder-text">${isInitial ? 'Processing...' : 'Updating preview...'}</p>`; const formData = new FormData(); formData.append('imageFile', currentFile); if (optionsForm) { const data = new FormData(optionsForm); for (let [key, value] of data.entries()) { const el = optionsForm.elements[key]; // Send only enabled inputs, exclude palette_selector
             if (el && !el.disabled && value !== '' && value !== null && key !== 'palette_selector') {
                 formData.append(key, value);
             } } } console.log("Sending data:", Object.fromEntries(formData));
         try { const res = await fetch('/convert', { method: 'POST', body: formData }); const result = await res.json(); if (!res.ok) throw new Error(result.error || `Server error: ${res.status}`); if (result.svg) { currentSvgContent = result.svg; if (svgOutputDiv) { svgOutputDiv.innerHTML = currentSvgContent; svgOutputDiv.classList.remove('placeholder-text'); } updateStatus('Complete!', 'success', 3000); if (downloadBtn) downloadBtn.disabled = false; if (convertBtn) { convertBtn.textContent = 'Update Vectorization'; convertBtn.disabled = true; // Disable update button *after* successful conversion
              } if (originalImageNaturalDims.width > 0) setTimeout(calculateAndApplyWrapperSize, 50); else console.warn("Orig dims missing after convert."); if(isInitial) resetZoomPan(); } else throw new Error("No SVG data received from server."); } catch (error) { console.error('Conversion Failed:', error); updateStatus(`Error: ${error.message}`, 'error'); if(svgOutputDiv){ if(isInitial) resetResultArea(false); else svgOutputDiv.innerHTML = `<p class="placeholder-text error-text">Update Failed</p>`; } if (downloadBtn) downloadBtn.disabled = true; if (convertBtn) convertBtn.disabled = !currentFile; // Re-enable only if there's a file
    ...
    // --- Option Reset & Default Storage ---
    function storeDefaultOptions() { if (!optionsForm) return; const data = new FormData(optionsForm); for (let [key, value] of data.entries()) { defaultOptions[key] = value; } // Ensure color_precision matches palette_selector in defaults
        if (defaultOptions.hasOwnProperty('palette_selector')) { defaultOptions['color_precision'] = defaultOptions['palette_selector']; } // Assign collected defaults to the "General / Balanced" preset object
        const defaultPreset = presets.find(p => p.name === "General / Balanced"); if (defaultPreset) { defaultPreset.options = {...defaultOptions}; } else { console.error("Could not find 'General / Balanced' preset to store defaults!"); } console.log("Stored defaults:", defaultOptions); }
    function handleResetOptions() { if (!optionsForm) return; applyOptions(defaultOptions); updateStatus('Options reset to default.', 'success', 2000); resetPresetSelection(); }

    // --- Preset Logic ---
    function populatePresetSelect() { if (!presetSelect) return; presetSelect.innerHTML = '<option value="" disabled selected>Load Preset...</option>'; // Changed placeholder
         presets.forEach((preset, index) => { const option = document.createElement('option'); option.value = index.toString(); option.textContent = preset.name; presetSelect.appendChild(option); }); }
    ...
    function resetPresetSelection() { // Only reset if it's not already the placeholder
        if(presetSelect && presetSelect.value !== "") presetSelect.value = ""; }

    // --- Apply Options Logic (Used by Presets, Reset, and Load) ---
    function applyOptions(optionsToApply) { if (!optionsForm) return; console.log("Applying options:", optionsToApply); let needsUpdateAvailability = false; for (const key in optionsToApply) { const value = optionsToApply[key]; const element = optionsForm.elements[key]; if (element) { // Check if element exists
            if (element.type === 'radio' || element.type === 'checkbox') { /* Handle if needed */ } else { element.value = value; } // Sync number input if linked range changed
            if (element.type === 'range') { const numInputId = `${element.id}Num`; const numInput = document.getElementById(numInputId); if (numInput) { numInput.value = value; } } // Flag if critical display options changed
            if (key === 'mode' || key === 'color_mode' || key === 'palette_selector') { needsUpdateAvailability = true; } // Trigger a 'change' event on the element *after* setting value
            // Need a slight delay for the browser to register the value change before firing event listeners
            setTimeout(() => element.dispatchEvent(new Event('change', { bubbles: true })), 0); } else { console.warn(`Option key "${key}" with value "${value}" not found in form.`); } } // After loop, update availability and enable button
         setTimeout(() => { if (needsUpdateAvailability) { updateOptionsAvailability(); } // Enable update button if an image is loaded
            if (currentFile && convertBtn) { convertBtn.disabled = false; convertBtn.textContent = 'Update Vectorization'; } }, 50); // Delay ensures changes are processed
     }

    // --- Save/Load Options Logic ---
    function handleSaveOptions() { if (!optionsForm || !downloadLink) return; const currentOptions = {}; const formData = new FormData(optionsForm); // Use defaultOptions keys as the canonical list
         Object.keys(defaultOptions).forEach(key => { if (formData.has(key)) { currentOptions[key] = formData.get(key); } }); try { const jsonString = JSON.stringify(currentOptions, null, 2); const blob = new Blob([jsonString], { type: 'application/json;charset=utf-8' }); const url = URL.createObjectURL(blob); downloadLink.href = url; downloadLink.download = 'vectorise-options.json'; downloadLink.click(); URL.revokeObjectURL(url); updateStatus('Settings saved.', 'success', 2000); } catch (e) { console.error('Error saving options:', e); updateStatus('Error saving settings.', 'error'); } }
    function handleLoadOptionsFile(event) { if (!event.target.files || event.target.files.length === 0) return; const file = event.target.files[0]; if (file.type !== 'application/json') { updateStatus('Error: Please select a valid .json settings file.', 'error'); optionsFileInput.value = ''; return; } const reader = new FileReader(); reader.onload = (e) => { try { const loadedOptions = JSON.parse(e.target.result); if (typeof loadedOptions !== 'object' || loadedOptions === null || Array.isArray(loadedOptions)) { throw new Error("Invalid JSON structure."); } const knownKeys = Object.keys(defaultOptions); const loadedKeys = Object.keys(loadedOptions); const hasKnownKey = knownKeys.some(key => loadedKeys.includes(key)); if (!hasKnownKey) { throw new Error("JSON does not contain recognizable options."); } applyOptions(loadedOptions); updateStatus('Settings loaded successfully.', 'success', 3000); resetPresetSelection(); } catch (error) { console.error('Error loading options file:', error); updateStatus(`Error loading settings: ${error.message}`, 'error'); } finally { optionsFileInput.value = ''; } }; reader.onerror = () => { updateStatus('Error reading settings file.', 'error'); optionsFileInput.value = ''; }; reader.readAsText(file); }
    ...
    // --- Utility Functions ---
    let statusClearTimer; function updateStatus(message, type, clearDelay = 0) { const target = appView && !appView.classList.contains('hidden') ? statusArea : landingStatusArea; if(!target) return; clearTimeout(statusClearTimer); target.textContent = message; target.className = `status-area ${appView && !appView.classList.contains('hidden') ? 'app-status' : 'landing-status'}`; if (type) { target.classList.add(type); } if(type!=='error'&&clearDelay>0&&message!=='') { statusClearTimer=setTimeout(()=>{ if(target.textContent===message) { target.textContent = ''; target.className = `status-area ${appView && !appView.classList.contains('hidden') ? 'app-status' : 'landing-status'}`; } }, clearDelay); } if(type === 'error'){ console.error("UI Status:", message); } }
    ...
    updateZoomButtons();
    console.log("vectorise.me script initialized (Presets Renamed, Examples Added).");
