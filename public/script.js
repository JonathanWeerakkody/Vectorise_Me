// public/script.js - Multi-Page Logic (App + Contact Form)
...
document.addEventListener('DOMContentLoaded', () => {
     ...
     const presets = [
        ...
        },
        {
            ...
            options: {
                ...
                palette_selector: "8", color_precision: "8", gradient_step: "1",
                mode: "spline", corner_threshold: "40",
                path_precision: "3", spline_threshold: "0.75", splice_threshold: "45", segment_length: "2"
            }
        },
        {
    ...
    function handleOptionsFormChange() {
        // This function now primarily enables the update button and syncs color precision.
        // Option availability and preset deselection are handled separately if needed.
        ...
        // updateOptionsAvailability(); // This might be too aggressive here, called by applyOptions
        // resetPresetSelection(); // Deselect preset on manual change
    }
    ...
    function calculateAndApplyWrapperSize() { if (!previewAreaWrapper || !previewOriginalImage || !originalImageWrapper || !vectorImageWrapper) return; // Only run if elements exist
        if (originalImageNaturalDims.width === 0 && previewOriginalImage.naturalWidth > 0) { // Try to get dims if missing
    ...
    function handlePresetChange(event) { if (!appView) return; const selectedIndex = event.target.value; if (selectedIndex === "" || !presets[selectedIndex]) return; const selectedPreset = presets[selectedIndex]; applyOptions(selectedPreset.options); updateStatus(`Preset "${selectedPreset.name}" loaded.`, 'info', 3000, true); }
    function resetPresetSelection() { if(presetSelect && presetSelect.value !== "") presetSelect.value = ""; }
    function applyOptions(optionsToApply) { if (!optionsForm) return; console.log("Applying options:", optionsToApply); let needsUpdateAvailability = false; for (const key in optionsToApply) { const value = optionsToApply[key]; const element = optionsForm.elements[key]; if (element) { if (element.type === 'radio' || element.type === 'checkbox') {} else { element.value = String(value); } if (element.type === 'range') { const numInputId = `${element.id}Num`; const numInput = document.getElementById(numInputId); if (numInput) { numInput.value = String(value); } } if (key === 'mode' || key === 'color_mode' || key === 'palette_selector') { needsUpdateAvailability = true; } setTimeout(() => { // Ensure change event fires after value is potentially set
             const changeEvent = new Event('change', { bubbles: true });
             element.dispatchEvent(changeEvent);
            }, 0); } else { console.warn(`Option key "${key}" with value "${value}" not found in form.`); } } setTimeout(() => { if (needsUpdateAvailability) { updateOptionsAvailability(); } if (currentFile && convertBtn) { convertBtn.disabled = false; convertBtn.textContent = 'Update Vectorization'; } }, 50); }
    ...
    function handleContactFormSubmit(event) {
        ...
        if (!isValid) {
         ...
         // Reset border colors if previously marked invalid
        ...
        try {
             ...
             showContactFormStatus("Please send the email using your email application.", "info");
             // Don't reset immediately, user needs to see the pre-filled email
             // contactForm.reset();
        } catch (error) {
            console.error("Failed to open mailto link:", error);
            showContactFormStatus("Could not open email client automatically. Please copy the details manually and send to " + recipientEmail, "error");
        }
    }
    ...
    function showContactFormStatus(message, type) {
        ...
        contactFormStatus.style.display = 'block'; // Make sure it's visible
    }
